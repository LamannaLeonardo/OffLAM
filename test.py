...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--domain", help="The domain to be tested. A (possibly empty) input domain pddl file must be "
                                       "contained into the 'Benchmarks' directory. A directory named as the input "
                                       "domain with the input traces must be contained into the 'Input traces' "
                                       "directory.", type=str)
    args = parser.parse_args()
    domain = args.domain

    assert os.path.exists(os.path.join('Analysis', 'Benchmarks', f'{domain}.pddl')), \
        f"Missing domain file {os.path.join('Analysis', 'Benchmarks', f'{domain}.pddl')}"

    assert os.path.exists(os.path.join('Analysis', 'Input traces', f'{domain}', Configuration.EXP)), \
        f"Missing input traces directory {os.path.join('Analysis', 'Input traces', f'{domain}', Configuration.EXP)}"

    assert len(os.listdir(os.path.join('Analysis', 'Input traces', f'{domain}', Configuration.EXP))) > 0, \
        f"No input traces found in directory {os.path.join('Analysis', 'Input traces', f'{domain}', Configuration.EXP)}"

    TRACES_DIR = 'Analysis/Input traces'

    domains = sorted(os.listdir(TRACES_DIR))

    try:
        run = len(os.listdir(os.path.join("Analysis/Results", domain, Configuration.EXP)))
    except:
        run = 0

    results_dir =os.path.join("Analysis/Results", domain, Configuration.EXP, f"run{run}")

    os.makedirs('PDDL', exist_ok=True)

    traces = os.listdir(f'{TRACES_DIR}/{domain}/{Configuration.EXP}')
    trace_names = [f'{TRACES_DIR}/{domain}/{Configuration.EXP}/{t}' for t in traces]

    # Copy input action model
    copyfile(os.path.join("Analysis/Benchmarks/", domain + ".pddl"),
             'PDDL/domain_input.pddl')

    print(f"\nProcessing input traces of domain {domain}")

    path_logs = results_dir
